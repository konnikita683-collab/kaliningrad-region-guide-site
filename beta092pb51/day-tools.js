(function(){
  ...
  var REGION={title:'Экскурсии по Калининградской области',meta:'подборка по теме и месту этого дня · выберите вариант со свободной датой',url:'https://experience.tripster.ru/experience/Kaliningrad/'};

  var byId={'kal-classic':KAL,'amalienau':KAL,'forts':KAL,'food':KAL,'zelenogradsk':ZEL,'slow':ZEL,'curonian':CUR,'rybachy-bike':CUR,'svetlogorsk':SVET,'yantarny':WEST,'coast':WEST,'baltiysk':WEST,'south':REGION,'east':REGION,'castles':REGION};
  ...
  function textPick(card){
    var head=q('.day-head',card)||q('h2,h3',card);var text=(head?head.innerText:card.innerText)||'';
  ...
  function addExcursions(){qa('.day').forEach(function(card,i){var r=pick(card,i),old=q('.guide-day-excursion',card),isBooked=booked(i);if(!old){old=document.createElement('div');old.className='guide-day-excursion';var head=q('.day-head',card)||q('h2,h3',card);if(head&&head.nextSibling)card.insertBefore(old,head.nextSibling);else card.insertBefore(old,card.firstChild);}var sig=r.url+'|'+(isBooked?'1':'0');if(old.dataset.sig===sig)return;old.dataset.sig=sig;old.dataset.url=r.url;old.classList.toggle('is-booked',isBooked);old.innerHTML=excursionHtml(r,isBooked);var btn=q('.guide-booked-toggle',old);if(btn)btn.addEventListener('click',function(){localStorage.setItem(bookedKey(i),isBooked?'0':'1');old.dataset.sig='';addExcursions();});});}
  function addNotes(){qa('.day').forEach(function(card,i){var box=q('.guide-day-notes',card);if(!box){box=document.createElement('details');box.className='guide-day-notes';box.innerHTML='<summary>Моя заметка к этому дню</summary><textarea rows="3" placeholder="Например: экскурсия на 10:00, место встречи, номер брони, ресторан…"></textarea>';card.appendChild(box);var ta=q('textarea',box);ta.value=localStorage.getItem(noteKey(i))||'';ta.addEventListener('input',function(){localStorage.setItem(noteKey(i),ta.value);});}else{var ta=q('textarea',box),saved=localStorage.getItem(noteKey(i))||'';if(document.activeElement!==ta&&ta.value!==saved)ta.value=saved;}});}
  ...
  document.addEventListener('change',function(e){var t=e.target;if(t&&(t.closest&&t.closest('.day')||t.tagName==='SELECT')){setTimeout(schedule,80);}},true);
  var tries=0;(function boot(){attachObserver();schedule();if(++tries<16)setTimeout(boot,400);})();window.guideRefreshDayTools=refresh;
})();