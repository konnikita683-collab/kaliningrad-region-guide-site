(function(){
  ...
  var routes={
    ...
    'Светлогорск':{yandex:'svetlogorsk',ostrovok:'svetlogorsk',ostrovokId:602214},
  ...
  function installCommerceFix(){
    ...
    window.guideCommerce.buildOstrovokUrl=function(ctx){
      ...
      var guests=String(adults);if(ages.length)guests+='and'+ages.join('.');
      u=append(u,'guests',guests);u=append(u,'search','yes');if(ctx.ostrovokId||r.ostrovokId)u=append(u,'q',ctx.ostrovokId||r.ostrovokId);
      return u;
    };
    window.guideCommerce.buildYandexTravelHotelUrl=function(ctx){
      ...
      if(ctx.start)u=append(u,'checkin_date',ctx.start);if(ctx.end)u=append(u,'checkout_date',ctx.end);if(ctx.adults)u=append(u,'adults',ctx.adults);
      var ages=cleanAges(ctx.childrenAges);if(!ages.length)ages=cleanAges(read('guideGuestAges',[]));var n=Math.max(0,+ctx.children||ages.length);if(n&&ages.length>=n)u=append(u,'children_ages',ages.slice(0,n).join(','));
