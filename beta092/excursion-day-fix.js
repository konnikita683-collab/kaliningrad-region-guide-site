(function(){
  ...
  try{
    ...
    window.__guideExcursionDayFixInstalled='EX2';
    ...
    function pick(card){var text=card.innerText||'';if(/День приезда|День вылета|День отъезда/i.test(text))return null;for(var i=0;i<recs.length;i++){var r=recs[i];if(r.rx.test(text)&&(!r.city||r.city.test(text)))return r;}return null;}
    function addDayCards(){qa('.day').forEach(function(card){var old=q('.guide-day-excursion',card),r=pick(card);if(!r){if(old)old.remove();return;}if(old&&old.dataset.url===r.url)return;if(old)old.remove();var box=document.createElement('div');box.className='guide-day-excursion';box.dataset.url=r.url;box.innerHTML='<div class="guide-day-excursion-kicker">Экскурсия по теме дня</div><b>'+r.title+'</b><span>'+r.meta+'</span><a href="'+r.url+'" target="_blank" rel="noopener" data-tripster-day>Посмотреть и забронировать</a>';var head=q('.day-head',card);if(head&&head.nextSibling)card.insertBefore(box,head.nextSibling);else if(head)card.appendChild(box);else card.insertBefore(box,card.firstChild);});}
    ...
    function addEventLinks(){qa('.event-card').forEach(function(card){if(q('.guide-event-ticket-link',card))return;var actions=q('.event-actions',card),a=document.createElement('a');a.className='guide-event-ticket-link';a.href=ticketUrl(card);a.target='_blank';a.rel='noopener';a.textContent='Билеты / подробнее';if(actions)actions.insertBefore(a,actions.firstChild);else card.appendChild(a);});}
    function cleanLegacyDayEventPromos(){qa('.day .event-recommendation').forEach(function(x){x.style.display='none';});}
    function refresh(){try{cleanLegacyDayEventPromos();addDayCards();addEventLinks();}catch(e){console.error('EX2 refresh',e);}}
    ...
    document.addEventListener('click',function(e){var t=e.target;if(t&&(t.dataset&&['days','events'].indexOf(t.dataset.tab)>=0||t.id==='next'))setTimeout(refresh,220);},true);
    var tries=0;(function boot(){refresh();if(++tries<12)setTimeout(boot,500);})();
    window.guideRefreshExcursions=refresh;
  }catch(err){console.error('EX2 init',err);}
})();